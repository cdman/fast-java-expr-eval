...
public class MathEval extends Object {
	...
	public MathEval() {
		...
		pureFunctions = new TreeMap<String, FunctionHandler>(
				String.CASE_INSENSITIVE_ORDER);
		impureFunctions = new TreeMap<String, FunctionHandler>(
				String.CASE_INSENSITIVE_ORDER);
	...
	public MathEval clear(String pfx) {
		variables.subMap((pfx + "."), (pfx + "." + Character.MAX_VALUE))
				.clear();
		return this;
	}
	 ...
	 */
	public double evaluate(String exp) throws NumberFormatException,
			ArithmeticException {
		expression = exp;
		isConstant = true;
		offset = 0;
	...
	public Set<String> getVariablesWithin(String exp) {
		...
		for (StringTokenizer tkz = new StringTokenizer(exp, separators, true); tkz
				.hasMoreTokens();) {
	 ...
	 */
	private double _evaluate(int beg, int end) throws NumberFormatException,
			ArithmeticException {
	 ...
	 */
	private double _evaluate(int beg, int end, double lft, Operator pnd,
			Operator cur) throws NumberFormatException, ArithmeticException {
		...
		for (ofs = beg; (ofs = skipWhitespace(expression, ofs, end)) <= end; ofs++) {
			...
			for (beg = ofs; ofs <= end; ofs++) {
				...
				if ((nxt = getOperator(chr)) != OPERAND) {
					if (nxt.internal) {
						nxt = OPERAND;
					} // must kill operator to prevent spurious
						// "Expression ends with a blank sub-expression" at end
						// of function
					else {
						break;
					}
...

			{
				...
				if (beg == ofs
						&& (cur.unary == LEFT_SIDE || nxt.unary == RIGHT_SIDE)) {
					...
					ofs = skipWhitespace(expression, offset + 1, end); // skip
																		...
																		// following
																		// whitespace
					nxt = (ofs <= end ? getOperator(expression.charAt(ofs))
							: OPERAND); // modify next operator
					...
					try {
						if (stringOfsEq(expression, beg, "0x")) {
							rgt = (double) Long.parseLong(
									expression.substring(beg + 2, ofs).trim(),
									16);
						} else {
							rgt = Double.parseDouble(expression.substring(beg,
									ofs).trim());
						}
		...
		if (ofs > end && cur != OPERAND) {
			if (cur.unary == LEFT_SIDE) {
				...
				throw exception(ofs,
						"Expression ends with a blank operand after operator '"
								+ nxt.symbol + "'");
			}
		}
		offset = ofs;
	...
	private double doFunction(int beg, int end) {
		...
		try {
			...
			fncargs = null;
			throw exception(beg, "Function not supported in this JVM: \""
					+ fncnam + "\"");
		} catch (UnsupportedOperationException thr) {
			fncargs = null;
			...
			fncargs = null;
			throw exception(beg,
					"Unexpected exception parsing function arguments", thr);
		} finally {
			if (fncargs != null) {
				if (fncargs.hasNext()) {
					throw exception(fncargs.getIndex(),
	...
	private ArithmeticException exception(int ofs, String txt, Throwable thr) {
		return new ArithmeticException(
				txt
						...
						+ "\""
						+ " (Cause: "
						+ (thr.getMessage() != null ? thr.getMessage() : thr
								.toString()) + ")");
	}
	...
	static public final class Operator extends Object {
		 ...
		 * and/or may be unary.
		 * <p>
		 * Using different precedence for one side allows afinity binding such
		 * that consecutive operators are evaluated left to right.
		 * <p>
		 * Flagging an operator as unary binds the precedence for the specified
		 * side such that it always has maximum precedence when considered from
		 * the opposite side.
		 */
		public Operator(char sym, int prclft, int prcrgt, int unibnd,
				OperatorHandler hnd) {
			this(sym, prclft, prcrgt, unibnd, false, hnd);

			if (prclft < 0 || prclft > 99) {
				throw new IllegalArgumentException(
						"Operator precendence must be 0 - 99");
			}
			...
			if (handler == null) {
				throw new IllegalArgumentException(
						"Operator handler is required");
			}
		}

		Operator(char sym, int prclft, int prcrgt, int unibnd, boolean intern,
				OperatorHandler hnd) {
			symbol = sym;
			precedenceL = prclft;
			precedenceR = prcrgt;
	...
	static public interface FunctionHandler {
		public double evaluateFunction(String fncnam, ArgParser fncargs)
				throws ArithmeticException;
	}
	 ...
	 */
	static class DefaultImpl extends Object implements OperatorHandler,
			FunctionHandler {
		private DefaultImpl() {
		}
		...
		public double evaluateOperator(double lft, char opr, double rgt) {
			...
			case '·':
				return lft * rgt; // multiply (because it's a Unicode world out
									// there)
			case '(':
				return lft * rgt; // multiply implicitly due to brackets (e.g
									// (a)(b)
			case '/':
		...
		static private final Operator OPR_EQU = new Operator('=', 99, 99,
				RIGHT_SIDE, true, DefaultImpl.INSTANCE); // simple assignment,
															// used as the final
															// operation, must
															// be maximum
															// precedence
		static private final Operator OPR_PWR = new Operator('^', 80, 81,
				NO_SIDE, false, DefaultImpl.INSTANCE); // power
		static private final Operator OPR_NEG = new Operator('±', 60, 60,
				RIGHT_SIDE, true, DefaultImpl.INSTANCE); // unary negation
		static private final Operator OPR_MLT1 = new Operator('*', 40,
				DefaultImpl.INSTANCE); // multiply (classical)
		static private final Operator OPR_MLT2 = new Operator('×', 40,
				DefaultImpl.INSTANCE); // multiply (because it's a Unicode world
										// out there)
		static private final Operator OPR_MLT3 = new Operator('·', 40,
				DefaultImpl.INSTANCE); // multiply (because it's a Unicode world
										// out there)
		static private final Operator OPR_BKT = new Operator('(', 40,
				DefaultImpl.INSTANCE); // multiply (implicit due to brackets,
										// e.g "(a)(b)")
		static private final Operator OPR_DIV1 = new Operator('/', 40,
				DefaultImpl.INSTANCE); // divide (classical computing)
		static private final Operator OPR_DIV2 = new Operator('÷', 40,
				DefaultImpl.INSTANCE); // divide (because it's a Unicode world
										// out there)
		static private final Operator OPR_MOD = new Operator('%', 40,
				DefaultImpl.INSTANCE); // remainder
		static private final Operator OPR_ADD = new Operator('+', 20,
				DefaultImpl.INSTANCE); // add/unary-positive
		static private final Operator OPR_SUB = new Operator('-', 20,
				DefaultImpl.INSTANCE); // subtract/unary-negative
	...
	static private final Operator OPERAND = new Operator('\0', 0, 0, NO_SIDE,
			false, null); // special "non-operator" representing an operand
							// character
